...
const RequestBox: FC<Props> = ({ request, setTrigger }) => {
    ...
    const { isPending, isBlocked, addBlock, removeBlock } = useBlockStatus(request._id);
    const { addFriend } = useFriendStatus(request._id);

    const handleAccept = async () => {
        const { statusCode, message } = await setRequest(user?.id!, request._id, false);
    ...
    const handleDecline = async () => {
        await setRequest(user?.id!, request._id, false);
        return setTrigger(prev => !prev);
    };
    ...
    return (
        <>
            <div className="flex p-3 items-center">
                <LazyLoadImage
                    onClick={() => navigate('/profile', { state: { userId: request._id } })}
                    src={request.image}
                    alt='request'
                    effect="blur"
