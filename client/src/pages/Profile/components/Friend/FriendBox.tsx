...
const FriendBox: FC<Props> = ({ friend }) => {
    ...
    const { isPending, isFriend, addFriend, removeFriend } = useFriendStatus(friend._id);
    const { isBlocked, addBlock, removeBlock } = useBlockStatus(friend._id);
    ...
    const handleClickMessage = async () => {
        const channelId = await checkIsChannelExist(user?.id!, friend._id);
    ...
    return (
        <>
            <div className="flex p-3 items-center">
                <LazyLoadImage
                    onClick={() => navigate('/profile', { state: { userId: friend._id } })}
                    src={friend.image}
                    alt='friend'
                    effect="blur"
