...
const Box: FC<Props> = ({ user }) => {
  ...
  const { isFriend, isPending, removeFriend } = useFriendStatus(user._id);
  const { isBlocked, addBlock, removeBlock } = useBlockStatus(user._id);

  const handleRequest = async () => {
    const { statusCode } = await setRequest(currentUser?.id!, user._id, true);
